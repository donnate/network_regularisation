...
class LogEstimator(BaseEstimator):
    def __init__(self, l1 = 0, l2 = 0, D = 0):
        self.l1 = l1
        self.l2 = l2
        self.D = D
...
class Log_OUR(LogEstimator):
    def fit(self, X, y):
        n, p = X.shape
        
        beta1 = cp.Variable(p)
        prob1 = cp.Problem(cp.Minimize(loss_logit(X, y, beta1) + EE_pen(beta1, self.l1, self.l2, self.D)))
        prob1.solve()
        self.beta = beta1.value
        return self
class Log_FL(LogEstimator):
    def fit(self, X, y):
        n, p = X.shape
        ...
        self.beta = beta1.value
        return self
class Log_SL(LogEstimator):
    def fit(self, X, y):
        n, p = X.shape
        
        beta1 = cp.Variable(p)
        prob1 = cp.Problem(cp.Minimize(loss_logit(X, y, beta1) + SL_pen(beta1, self.l1, self.l2, self.D)))
        prob1.solve()
        self.beta = beta1.value
        return self
