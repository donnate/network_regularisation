...
Testing folds_cv function

```{r}
N <- nrow(X)
folds = createFolds(1:N,k=2)

lambda1 = 10^seq(-3,2,1)
lambda2 = 10^seq(-3,2,1)
...
model = genglm(train_x,train_y,D,family='gaussian',l1,l2,solver='CD')
fitted_y = predict.genglm(model, test_x,type="response")
fold_score = mean((test_y-fitted_y)^2)
```

```{r}
fold_score = folds_cv(fold,X,y,D,family='gaussian',l1,l2,solver='CD')
```
...
```{r}
param_cv = function(lambdas, x, y, D, family, solver){
  l1 = lambdas[1,]$l1
  l2 = lambdas[2,]$l2

  folds_score = mean(unlist(lapply(folds, folds_cv, x=x, y=y, D=D, family=family, solver=solver, l1=l1, l2=l2)))
  return(folds_score)
}
```
...
vectorized.cv.genglm = function(x,
  ...
  lambdas = expand.grid(lambda1,lambda2)
  colnames(lambdas) = c('l1','l2')
  cv_scores = apply(lambdas, param_cv, x=x,y=y,D=D,family=family,solver=solver)
...
cv.genglm <- function(x,
  ...
  for(i in 1:nrow(lambdas)){
    ...
    for(fold in folds){
      ...
      model = genglm(train_x,train_y,D,family,l1,l2,solver)
      fitted_y = predict.genglm(model, test_x,type="response")
...
```{r}
genglm(X,y,D,family="gaussian",solver="CD")
```

```{r}
test = genglm(train_x,train_y,D,family='gaussian',solver='CD')
```
